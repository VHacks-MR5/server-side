...
def app_match():
	...
	db = sqlite3.connect(DATABASE)
	cursor = db.cursor()
	cursor.execute("SELECT first_name, last_name, trafficked FROM refugees WHERE picturepath = '{}'".format(url[:-4]))
	name = cursor.fetchall()
	if len(name) < 1:
		name.append(('your loved','one',1))
	if url[:3] == "000":
		url = 'https://familylinks.icrc.org/europe/PersonImages/' + url
	response = {'messages':[{'text': 'Similarity score is %s%%' % int((confidence*100))},{'text': 'We located {} {} '.format(name[0][0],name[0][1])}, {'attachment':  {"type": "image","payload": {"url": url.replace("dl=1", "")}}}]}
